...
class Matches extends Component {
  render() {
    return (
      ...
      .map((match, index) =>
        <div key={match.name} style={{width: '100%', maxWidth: '480px', margin: '0 auto', marginBottom: '12px', border: '1px solid #ccdae5', borderRadius: '12px', overflow: 'hidden'}}>
        ...
        <div style={{with: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px'}}>
          ...
          <div style={{width: '40%'}}>
            <div style={{textAlign: 'center'}}><b>{index <= 47 ? `Group ${match.group.toUpperCase()}` : index <= 55 ? 'Round of 16' : index <= 59 ? 'Quarterfinals' : index <= 61 ? 'Semifinals' : index === 62 ? 'Third Place Playoff' : 'Finals'}</b></div>
