      ...
      .map((match, index) =>
        ...
        <div style={{with: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px'}}>
          {match.home_team.name
            ...
            <img src={match.home_team.flag} style={{width: '64px', height: '64px', borderRadius: '50%', border: '1px solid grey', objectFit: 'cover'}} />
          ...
          <div style={{width: '40%'}}>
            ...
            <div>{index <= 47 ? `Group ${match.group.toUpperCase()}` : index <= 55 ? 'Round of 16' : index <= 59 ? 'Quarterfinals' : index <= 61 ? 'Semifinals' : index === 62 ? 'Third Place Playoff' : 'Finals'}</div>
            <div style={{display: 'flex', justifyContent: 'space-around'}}>
              <div style={{fontSize: '2em'}}>{match.home_result}</div>
              <div style={{fontSize: '2em'}}>{match.away_result}</div>
            </div>
          </div>
          {match.away_team.name
            ? <div style={{display: 'flex', flexFlow: 'column', width: '30%', justifyContent: 'center', alignItems: 'center'}}>
            <img src={match.away_team.flag} style={{width: '64px', height: '64px', borderRadius: '50%', border: '1px solid grey', objectFit: 'cover'}} />
